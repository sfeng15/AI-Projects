...
int one_node_calculation(int i, int j) {
	...
	if(grid[i][j].reward_initial!=0){
		grid[i][j].utility = grid[i][j].reward_initial;
	}

	float after_unity=grid[i][j].utility;

	if(  abs(before_unity - after_unity) < 0.05  ){
		// cout << i << endl;
		// cout << j << endl;
		// cout << endl;
...
int BFS(){
	...
	while(!myqueue.empty()){
	    ...
	    if(temp.visited == 0){
		    ...
		    if(currentX+1<6){
		    	if(grid[currentX+1][currentY].inlist==0 && grid[currentX+1][currentY].wall==false){
		    		grid[currentX+1][currentY].inlist=1;
		    		myqueue.push(grid[currentX+1][currentY]);
		    	}
		    }
		    if(currentY+1<6){
		    	if(grid[currentX][currentY+1].inlist==0 && grid[currentX][currentY+1].wall==false){
	...
	for(int i = 0; i < 6; i++){
		for(int j = 0; j < 6; j++){
			if(grid[i][j].utility * 100==0){
				...
				cout << setprecision(3) << grid[i][j].utility<< " ";			
			}
