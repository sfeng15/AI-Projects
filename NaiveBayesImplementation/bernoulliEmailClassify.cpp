...
void parseTraining(){
	...
	ifstream myEmailFile("train_email.txt");
	...
	while ( getline(myEmailFile,line) ){
		...
		if (myVector[0] == "1")
		{
			...
			for (int i = 1; i < myVector.size(); i++)
			{
				...
				{
					goodEmail[name] = goodEmail[name] + 1;
				}
				else{
					goodEmail[name] = 1;
				}
...
void parseTest(){
	...
	ifstream myEmailFile("test_email.txt");
	...
	while ( getline(myEmailFile,line) ){
		vector<string> myVector = split(line, ' ');
			...
			else if(realback==1 &&  estimateback==0){
				confusionMatrix[0][1] = confusionMatrix[0][1] +1;
			}
			else if(realback==0 &&  estimateback==1){
				confusionMatrix[1][0] = confusionMatrix[1][0] +1;
			}
			else if(realback==0 &&  estimateback==0){
